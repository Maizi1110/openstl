...
#     return np.mean(csi)
#
# def HSS(hits, fas, misses, cns, eps=1e-6):
#     """
#     Heidke Skill Score
#     Inputs:
#     Outputs:
#         hss = 2*(H*C - M*F) / ((H+M)*(M+C) + (H+F)*(F+C)) averaged over the D channels
#     """
#     hss_num = 2 * (hits * cns - misses * fas)
#     hss_den = (hits + misses) * (misses + cns) + (hits + fas) * (fas + cns)
...
#     eval_res = {}
#     eval_log = ""
#     allowed_metrics = ['mae', 'mse', 'rmse', 'ssim', 'psnr', 'snr', 'lpips', 'pod', 'sucr', 'csi','hss']
...
def sevir_metrics(pred, true, threshold):
    ...
    for i in range(pred.shape[0]):
        ...
        fas.append(np.sum((1 - t) * p))
        misses.append(np.sum(t * (1 - p)))
        cns.append(np.sum((1 - t) * (1 - p)))

    return np.array(hits), np.array(fas), np.array(misses), np.array(cns)
...
def metric(pred, true, mean=None, std=None, metrics=['mae', 'mse'],
    ...
    if any(m in metrics for m in ['pod', 'sucr', 'csi', 'hss']):
        ...
        for th in thresholds:
            ...
            if 'pod' in metrics:
                eval_res['pod' + suffix] = POD(hits, misses)
            if 'sucr' in metrics:
                eval_res['sucr' + suffix] = SUCR(hits, fas)
            if 'csi' in metrics:
                eval_res['csi' + suffix] = CSI(hits, fas, misses)
            if 'hss' in metrics:
                eval_res['hss' + suffix] = HSS(hits, fas, misses, cns)
