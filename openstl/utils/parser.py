...
def create_parser():
    ...
    parser.add_argument('--dataname', '-d', default='mmnist', type=str,
                        choices=['bair', 'mfmnist', 'mmnist', 'mmnist_cifar', 'noisymmnist', 'taxibj', 'human',
                                ...
                                'sevir', 'sevir_vis', 'sevir_ir069', 'sevir_ir107', 'sevir_vil', 'seivr_vil', 'cikm'],
    ...
    parser.add_argument('--method', '-m', default='SimVP', type=str,
                        choices=['ConvLSTM', 'convlstm', 'E3DLSTM', 'e3dlstm', 'MAU', 'mau', 'MIM', 'mim', 
                                ...
                                'SwinLSTM', 'swinlstm', 'swinlstm_d', 'swinlstm_b', 'exprecast','stldm','alphapre'],
    ...
    parser.add_argument('--save_root', default='./results/custom_test', type=str,
                        help='Root dir for custom evaluation reports')
    parser.add_argument('--run_id', default=None, type=str,
                        help='Run id used in custom evaluation report names')
    parser.add_argument('--metrics', dest='metric_set', default='main', type=str,
                        help='Metric set for custom evaluation: main | all | comma list')

    # �?鏍稿績淇敼锛氭坊鍔?img_size 鍙傛暟锛岀敤浜?AlphaPre 绛夋ā鍨嬪垵濮嬪寲棰戝煙缃戞牸
    parser.add_argument('--img_size', type=int, default=128, help='杈撳叆鍥惧儚鐨勫垎杈ㄧ巼澶у皬')
    return parser
